...
import { updateRecipe } from "@/lib/action/recipes";
...
export default function EditRecipeClient({ recipe }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    const res = await updateRecipe(recipe._id, { ...form, ingredients: valid, price: parseFloat(form.price) || 0 });
    if (res.error) {
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-6 md:p-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="md:col-span-2">
            ...
            <input type="text" name="recipeName" value={form.recipeName} onChange={handleChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700" placeholder="Recipe name" required />
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm font-medium mb-1">Image</label>
            <div className="flex items-center gap-4">
              {form.recipeImage && <img src={form.recipeImage} className="w-24 h-24 object-cover rounded-lg" alt="preview" />}
              <label className={`flex-1 flex items-center justify-center gap-2 border-2 border-dashed border-gray-300 dark:border-slate-600 rounded-lg p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700 ${uploading ? "opacity-50" : ""}`}>
                ...
                <span className="text-sm">{uploading ? "Uploading..." : "Change Image"}</span>
          ...
          <div>
            ...
            <select name="category" value={form.category} onChange={handleChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700">
          ...
          <div>
            ...
            <select name="cuisineType" value={form.cuisineType} onChange={handleChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700">
              <option value="">Select</option>
          ...
          <div>
            ...
            <select name="difficultyLevel" value={form.difficultyLevel} onChange={handleChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700">
              <option value="">Select</option>
          ...
          <div>
            ...
            <input type="text" name="preparationTime" value={form.preparationTime} onChange={handleChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700" placeholder="45 minutes" required />
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          <textarea name="instructions" value={form.instructions} onChange={handleChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 min-h-[150px] resize-y" required />
        </div>
        <div className="flex gap-4">
          <button type="button" onClick={() => router.back()} className="flex-1 border-2 border-gray-300 text-slate-600 px-8 py-3 rounded-xl font-semibold hover:bg-gray-50">
            Cancel
          </button>
          <button type="submit" disabled={submitting || uploading} className="flex-1 bg-gradient-to-r from-primary-500 to-primary-600 text-white px-8 py-3 rounded-xl font-semibold disabled:opacity-50">
