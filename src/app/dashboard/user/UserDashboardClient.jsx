...
export default function UserDashboardClient({ user, stats: initialStats }) {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (data.totalRecipes !== undefined) setStats(data);
      } catch {}
    };
    fetchStats();
  }, []);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {user.isPremium && (
          <span className="flex items-center gap-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-white px-4 py-2 rounded-full font-medium text-sm">
      ...
      {!user.isPremium && (
        <div className="bg-gradient-to-r from-primary-500 to-secondary-500 rounded-2xl p-6 mb-8 text-white">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              <h3 className="text-xl font-bold flex items-center gap-2"><Crown className="w-6 h-6" />Upgrade to Premium</h3>
              <p className="text-white/80 text-sm">Unlock unlimited recipes & premium badge!</p>
            </div>
            <button onClick={handlePremium} className="bg-white text-primary-600 px-8 py-3 rounded-xl font-bold hover:bg-primary-50">
              Get Premium - $9.99
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        {[
          { label: "Total Recipes", value: stats.totalRecipes, icon: UtensilsCrossed },
          { label: "Favorites", value: stats.totalFavorites, icon: Heart },
          { label: "Likes", value: stats.totalLikes, icon: ThumbsUp },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { href: "/dashboard/user/add-recipe", label: "Add Recipe", icon: UtensilsCrossed },
